...
echo
sudo apt-get install -y linux-image-$LATEST linux-headers-$LATEST \
    linux-headers-$LATEST-generic
sudo apt-get install -y linux-image-$LATEST-lowlatency \
...
for i in $OLD
do
    sudo apt-get purge -y $(dpkg --get-selections | grep linux \
