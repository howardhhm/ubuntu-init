...
sed -i -e 's|Exec=/opt/sublime_text/sublime_text %F|Exec=bash -c '`
`'"LD_PRELOAD=/opt/sublime_text/libsublime-imfix.so exec '`
`'/opt/sublime_text/sublime_text %F"|g' \
    /usr/share/applications/sublime_text.desktop
sed -i -e 's|Exec=/opt/sublime_text/sublime_text -n|Exec=bash -c '`
`'"LD_PRELOAD=/opt/sublime_text/libsublime-imfix.so exec '`
`'/opt/sublime_text/sublime_text -n"|g' \
    /usr/share/applications/sublime_text.desktop
sed -i -e 's|Exec=/opt/sublime_text/sublime_text --command '`
